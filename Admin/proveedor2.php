...
<table width="800" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	@menu_gestion($act_men_proveedor,$act_men_pro2,$nom_boton_lis);
	if($cod_env_ges>0){
		//Tabla Informacion Proveedor
		@imp_datos_gestion($txt_contacto,$list_prov,$cant_prov,$cod_env_ges,$txt_mov,$txt_vendedor,$cod_fac_asi);	    
		//Tabla no existe informacion
		//Llamo a funcion busca valor de numero de registros
		$num_asiento_cont 	= @busca_valor_asiento($cant_prov,$list_prov,$cod_env_ges);	
		//Tabla modificacion
		...
		if($num_asiento_cont<=0){
			...
			for($conta=1; $conta<=$num_asiento_cont; $conta++){
				@listado_numero_gestion($pos1, $pos2,$list_fac_tot,$pos_lista,$pagina_actual,$limitInf);
				$pos1++;
				$pos2++;
				$pos_lista++;
			}	
